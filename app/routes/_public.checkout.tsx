...
export async function loader({ request }: LoaderArgs) {
  ...
  const clientSecret = data?.cart?.payment_sessions[0].data.client_secret

  console.log('SEEECRET', clientSecret)

  return { clientSecret, cartId }
}

export default function CheckoutRoute() {
  const  { clientSecret, cartId } = useLoaderData<typeof loader>();
  ...
  return (
    <div className="w-full mt-8">
      ...
      {clientSecret && (
        ...
        <PaymentContainer clientSecret={clientSecret} cartId={cartId}/>
      </Elements>
      )}
    </div>
    </div>
